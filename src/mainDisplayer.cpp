...
static void help() {
  printf(
      ...
      "		-c=<path>			# the path to point cloud "
      "storage in *.ply format\n"
      "\n");
}
...
int main(int argc, char** argv) {
  ...
  if (points.empty()) {
    std::cout << "ERROR. Cannot load point cloud storage by path:"
              << pointsCloudStoragePath << std::endl;
    help();
    return -1;
  }
  ...
  myWindow.spin();
#else
  std::cout << "Your opencv version compiled without viz module. Please "
               "rebuild opencv with vtk and try again.";
#endif
  return 0;
}
