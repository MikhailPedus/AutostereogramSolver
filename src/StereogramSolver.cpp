...
cv::Mat stereogramSolver::reconstructDepthMap(const cv::Mat& stereogram, int countThreads) {
	if (stereogram.empty()) {
		std::cout << "ERROR. cv::Mat stereogramSolver::reconstructDepthMap(const cv::Mat& stereogram) get empty Mat..." << std::endl;
	...
	for (int i = 0; i < m_countThreads; i++) {
		chunksStereogram.push_back(stereogram(
			cv::Rect(0, 
				i*step, 
				stereogram.cols, 
				i == m_countThreads - 1 ? stereogram.rows - i*step : i*step + step < stereogram.rows ? step : stereogram.rows - i*step)));
	}
	...
	for (int i = 0; i < chunksStereogram.size(); i++) {
		m_threads.push_back(new std::thread(&stereogramSolver::reconstructDepthMapPartImg, 
			this,
			chunksStereogram[i],
			chunksDepthMap[i],
			offset,
	...
	cv::Mat depthMap(stereogram.rows, stereogram.cols-offset, CV_8UC1, cv::Scalar::all(255));
	int yOffset = 0;
...
int stereogramSolver::findRepeatOffset(const cv::Mat& stereogram) const {
	if (stereogram.empty()) {
		std::lock_guard<std::mutex> locker(m_PrintLocker);
		std::cout << "ERROR. \tint stereogramSolver::findRepeatOffset(const cv::Mat& stereogram)\t get empty Mat..." << std::endl;
		return -1;
	}
	int minDifference = -1;
	int bestStep = -1;
...
void stereogramSolver::reconstructDepthMapPartImg(const cv::Mat partOfStereogram, cv::Mat* outDepthMap, int offset, int threadIndex) {
	if (partOfStereogram.empty() || offset < 1) {
		std::lock_guard<std::mutex> locker(m_PrintLocker);
		std::cout << "ERROR. \tvoid stereogramSolver::reconstructDepthMapPartImg(const cv::Mat& partOfStereogram, cv::Mat& outDepthMap, int offset)\t invalid input param..." << std::endl;
		return;
	}
	
	*outDepthMap = cv::Mat(cv::Size(partOfStereogram.cols - offset, partOfStereogram.rows), CV_8UC1, cv::Scalar::all(0));
	
	int completePrecent = 0;
	for (int r = 0; r < partOfStereogram.rows; r++) {
		for (int c = 0; c < partOfStereogram.cols - offset - MATCH_BLOCKS_WIDTH; c++) {
			...
			pixelAfterOffset.push_back(image.at<unsigned char>(r, (c + offset) * image.channels() + nChannel));
			}*/
			int bestDepth = 0;
			double bestDiff = -1;
			for (int depth = 0; depth < offset / 2; depth++) {
				...
				for (int nChannel = 0; nChannel < partOfStereogram.channels(); ++nChannel) {
					...
					cv::Mat diffMat;
					cv::absdiff(partOfStereogram(cv::Rect(c + depth, r, MATCH_BLOCKS_WIDTH, 1)), partOfStereogram(zoneAfterOffset), diffMat);
					cv::Scalar diff = cv::sum(diffMat);
					for (int i = 0; i < diff.channels; ++i) {
						currentDiff += diff[i];
					}
					//					currentDiff += std::abs(image.at<unsigned char>(r, (c + depth) * image.channels() + nChannel) - pixelAfterOffset[nChannel]);
				}
		...
		int currentCompletePrecent = static_cast<int>(static_cast<double>(r) / static_cast<double>(partOfStereogram.rows) * 100);
